...
function startForDoc(data) {
    ...
    try {
        currentDoc.documentColorSpace = DocumentColorSpace.CMYK;
    } catch (eColor) {}
    return "200;文件打开成功";
}
...
function resolveLineSpec(mmL, mmS, pw, ph) {
    var expectedW = mm2pt(mmL, false);
    var expectedH = mm2pt(mmS, false);
    var tol = Math.max(2, mm2pt(8, false));
    var diff1W = Math.abs(pw - expectedW);
    var diff1H = Math.abs(ph - expectedH);
    var diff2W = Math.abs(pw - expectedH);
    var diff2H = Math.abs(ph - expectedW);
    var isSameFlip1 = diff1W < tol && diff1H < tol;
    var isSameFlip2 = diff2W < tol && diff2H < tol;
    var diff1 = diff1W + diff1H;
    var diff2 = diff2W + diff2H;
    if (isSameFlip1 || isSameFlip2) {
        return {
            mmL: mmL,
            mmS: mmS,
            flip: isSameFlip2 && !isSameFlip1,
            fromActual: false,
            matchPriority: 0,
    ...
    return {
        mmL: Math.max(pt2mm(pw), pt2mm(ph)),
        mmS: Math.min(pt2mm(pw), pt2mm(ph)),
        flip: pw < ph,
        fromActual: true,
        matchPriority: 1,
        diff: Math.min(diff1, diff2),
...
function getArtLines() {
    ...
    for (var i = 0; i < group.length; i++) {
        ...
        if (art) {
            ...
            if (!lineCandidatesByArt[art]) {
                lineCandidatesByArt[art] = [];
            }
            lineCandidatesByArt[art].push({
                group: p,
                width: pw,
                height: ph,
                mmCx: mmCx,
                bounds: pBounds,
                spec: resolveLineSpec(mmL, mmS, pw, ph)
            });
        }
    }
    for (var art in currentCutMap) {
        var candidates = lineCandidatesByArt[art] || [];
        candidates.sort(function(a, b) {
            if (a.spec.matchPriority != b.spec.matchPriority) {
                return a.spec.matchPriority - b.spec.matchPriority;
            }
            if (Math.abs(a.spec.area - b.spec.area) > 0.01) {
                return b.spec.area - a.spec.area;
            }
            return a.spec.diff - b.spec.diff;
        });
        var needCount = +(currentCutMap[art].kinds || 0);
        if (!(needCount > 0)) needCount = candidates.length;
        for (var ci = 0; ci < candidates.length && ci < needCount; ci++) {
            var cand = candidates[ci];
            currentLineCount++;
            cand.group.name = 'mqx_' + currentLineCount;
            if (cand.spec.fromActual) {
                currentCutMap[art].mmL = cand.spec.mmL;
                currentCutMap[art].mmS = cand.spec.mmS;
            }
            currentCutMap[art]['lines'][cand.group.name] = {
                ...
                itemsIndexToGroup: [],
                bounds: {
                    minX: cand.bounds[0] - mm2pt(cand.mmCx, false),
                    minY: cand.bounds[1] - mm2pt(cand.mmCx, false),
                    maxX: cand.bounds[2] + mm2pt(cand.mmCx, false),
                    maxY: cand.bounds[3] + mm2pt(cand.mmCx, false)
                }
            };
        }
    }
...
function renameAllItems() {
    ...
    for (var i = 0; i < totalCount; i++) {
        ...
        if (!isDieLineStroke(p) && !isValidMqxName(p.name) 
...
function main(data) {
    try {
        ...
        var fileMetaParts = failName.split("^");
        ...
        if (!fileDims) {
            throw new Error("文件名尺寸格式错误:" + failName);
        }
        if (artboardsLength == 1) {
            ...
            for (var i = 0; i < artboardsLength; i++) {
                ...
                var artMetaParts = String(artName || "").split("^");
                var artDims = artMetaParts.length > 6 ? parseSizeToken(artMetaParts[6]) : fileDims;
                var artKinds = +(artMetaParts[2] || 0);
                if (!(artKinds > 0)) artKinds = fileKinds;
                if (!artDims) artDims = fileDims;
                if (!artName || artName.indexOf("^") < 0) artName = failName + "_ART" + (i + 1);
