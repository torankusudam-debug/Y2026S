...
def pack_mix_by_height_rule(mix_types, page_max_h=PAGE_H_MAX):
    ...
    """
    X_START = float(PAGE_MARGIN_L)             # 左侧起排 x
    X_END = float(PAGE_W - PAGE_MARGIN_R)      # 右侧可用终点 x
    X_SPLIT = float(SPLIT_X)                   # 左区终点 x=320
    X_GAP_START = float(SPLIT_X + SPLIT_GAP_W) # 右区起点 x=330
    ...
    def _build_one_row(work_list, base_h, y):
        ...
        """
        x = X_START
        in_left = True              # True=当前在左区，False=当前在右区
        row_blocks = []
        row_added = []

        while True:
            region_end = X_SPLIT if in_left else X_END
            rem_w = float(region_end) - float(x)  # 当前区域剩余可用宽度
            ...
            for t in work_list:
                ...
                stack_cap = int(math.floor((base_h + gap) / (Hi + gap))) if Hi > 0 else 1  # 该列最多向下堆几张
                if stack_cap <= 0:
                    stack_cap = 1

                max_cols = int(math.floor((rem_w + gap) / (Wi + gap))) if Wi > 0 else 0    # 横向最多可摆几列
                if max_cols <= 0:
                    continue

                need_cols = int(math.ceil(float(t["rem"]) / float(stack_cap)))  # 把剩余张数摆完至少需要几列
                cols = min(max_cols, need_cols)
                if cols <= 0:
                    cols = 1

                block_w = float(cols) * Wi + float(max(0, cols - 1)) * gap
                leftover = rem_w - block_w  # 摆完该块后，当前区剩余宽度

                rank = (-Hi, float(leftover), -Wi)  # 优先高块、其次少浪费、再其次宽块
    ...
    while True:
        ...
        while True:
            ...
            cand_hs = hs[:6] if len(hs) > 6 else hs  # 只试前6个可能的 base_h，减少计算量
            ...
            for bh in cand_hs:
                ...
                density = used_area / denom  # 这一行的面积利用率

                score = (-density, float(leftover), float(row_used_h))  # 优先密度高、再优先剩余少、再优先行高小
...
def _mix_hole_fill(page, types_list, page_max_h, gap):
    ...
    cand_types.sort(key=lambda t: -(float(t["W"]) * float(t["H"])))  # 优先补大块
