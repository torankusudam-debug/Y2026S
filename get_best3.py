...
# =========================
MIX_ENABLE = True
MIX_THRESHOLD = 10  # N < 10 -> mix

LEFT_BLOCK_X0 = 0.0
LEFT_BLOCK_X1 = SINGLE_SPLIT_X                 # 320
RIGHT_BLOCK_X0 = SINGLE_SPLIT_X + SINGLE_SPLIT_GAP_W  # 330
RIGHT_BLOCK_X1 = SINGLE_W                      # 600

def _mix_x_starts_left(w):
    # 左区：从 5 开始，到 320-QR_W 结束（给右上角 QR 留 10mm）
    x0 = SINGLE_MARGIN_L
    x1 = LEFT_BLOCK_X1 - QR_W
    if w <= 0:
        return []
    cols = int(math.floor((x1 - x0) / float(w)))
    return [x0 + i * float(w) for i in range(max(0, cols))]

def _mix_x_starts_right(w):
    # 右区：从 330 开始，到 600-QR_W 结束（QR 贴在 590~600）
    x0 = RIGHT_BLOCK_X0
    x1 = RIGHT_BLOCK_X1 - QR_W
    if w <= 0:
        return []
    cols = int(math.floor((x1 - x0) / float(w)))
    return [x0 + i * float(w) for i in range(max(0, cols))]

def _mix_x_starts_full(w):
    # 全宽：左区 + 右区拼起来（仍保留 320~330 空白带）
    return _mix_x_starts_left(w) + _mix_x_starts_right(w)

# =========================
# 混拼候选（tight版：不按464插空，排更紧凑）
# =========================
def _build_mix_candidate(outer_w, outer_h, N, mode):
    """
    mode: 'L' 左区(0~320), 'R' 右区(330~600), 'F' 全宽(含中缝)
    tight：y=QR_BAND + rr*h，不做464插空（更紧凑）
    """
    if mode == "L":
        x_starts = _mix_x_starts_left(outer_w)
        bx0, bx1 = LEFT_BLOCK_X0, LEFT_BLOCK_X1
    elif mode == "R":
        x_starts = _mix_x_starts_right(outer_w)
        bx0, bx1 = RIGHT_BLOCK_X0, RIGHT_BLOCK_X1
    else:
        x_starts = _mix_x_starts_full(outer_w)
        bx0, bx1 = 0.0, float(SINGLE_W)

    if not x_starts:
        return None

    max_cols = len(x_starts)

    # ✅ 最少切刀思路仍保留：能一行放完优先一行，否则尽量少行
    if max_cols >= int(N):
        cols = int(N)
        rows = 1
    else:
    ...
    for idx in range(int(N)):
        rr = idx // cols
        cc = idx % cols
        y = float(QR_BAND) + float(rr) * float(outer_h)   # ✅ tight：不插464空隙
        placements.append({
            "x": float(x_starts[cc]),
            "y": y,
            "w": float(outer_w),
    ...
    H = float(max(p["y"] + p["h"] for p in placements)) if placements else float(QR_BAND)
...
def solve_mix_candidates(outer_w, outer_h, N):
    """
    返回 {'L':cand,'R':cand,'F':cand}，允许旋转
    评价：优先 rows=1，其次 H 更小
    """
    best = {"L": None, "R": None, "F": None}
    ...
    for ori in (0, 1):
        ...
        for mode in ("L", "R", "F"):
            cand = _build_mix_candidate(w, h, int(N), mode)
            if not cand:
                continue
            cand["ori"] = int(ori)
            ...
            if prev is None:
                ...
                key_new = (cand["rows"] != 1, cand["H"])
                key_old = (prev["rows"] != 1, prev["H"])
                if key_new < key_old:
                    best[mode] = cand
...
def solve_mix_candidates(outer_w, outer_h, N):
    """
    给一个类型（N<10），返回 L/R/F 三个 mode 的最佳候选（考虑旋转）。
    """
    best = {"L": None, "R": None, "F": None}
    ...
    for ori in (0, 1):
        ...
        for mode in ("L", "R", "F"):
            cand = _build_mix_candidate(w, h, N, mode)
            if not cand:
                continue
            cand["ori"] = int(ori)
            # 记录旋转
            ...
            # 评价：优先 rows=1（少横切层），其次 H 小（省纸），再其次 mode 优先 R（右区窄，先占它）
            prev = best[mode]
            if prev is None:
                best[mode] = cand
            else:
                key_new = (cand["rows"] != 1, cand["H"], 0 if mode == "R" else 1)
                key_old = (prev["rows"] != 1, prev["H"], 0 if mode == "R" else 1)
                if key_new < key_old:
                    best[mode] = cand
...
def _draw_label_top_left(page, text, x_left_mm, y_top_mm, max_w_mm):
    ...
    ypt = mm_to_pt(float(y_top_mm) + 7.5)  # 放在顶部 10mm band 内更稳
    if fontfile:
...
def _draw_mix_block(page, seg, cand, img_bytes, yoff_mm):
    """
    在 yoff_mm 这层里画一个“类型块”（左/右/全宽之一）
    """
    bx0 = float(cand["bx0"])
    bx1 = float(cand["bx1"])
    ...
    if seg.get("qr_bytes"):
        qr_rect = fitz.Rect(
            mm_to_pt(bx1 - QR_W), mm_to_pt(yoff_mm),
            mm_to_pt(bx1), mm_to_pt(yoff_mm + QR_H)
        )
        page.insert_image(qr_rect, stream=seg["qr_bytes"], keep_proportion=True)

    # Label：该类型块左上角（避开 QR）
    label_text = seg.get("label_text") or ""
    if label_text:
        _draw_label_top_left(page, label_text, bx0, yoff_mm, max_w_mm=(bx1 - bx0 - QR_W))

    # 贴图：按 placements（y 需要加 yoff）
    for p in cand["placements"]:
        x = float(p["x"])
        y = float(yoff_mm) + float(p["y"])
        w = float(p["w"])
        h = float(p["h"])
        ...
        if img_bytes:
            page.insert_image(inner, stream=img_bytes, keep_proportion=True,
                              rotate=(90 if p.get("rot") else 0))

def _mix_strip_height(strip):
    h = 0.0
    if strip.get("left"):
        h = max(h, float(strip["left"]["cand"]["H"]))
    if strip.get("right"):
        h = max(h, float(strip["right"]["cand"]["H"]))
    if strip.get("full"):
        h = max(h, float(strip["full"]["cand"]["H"]))
    return float(h)

def _build_mix_strips(mix_items):
    """
    mix_items: 每个元素包含 {seg, cands}，cands={'L','R','F'}
    输出 strips：每层最多左右各一个“单行块”，多行块独占一层
    """
    pairable_L = []
    pairable_R = []
    flexible = []
    ...
    for it in mix_items:
        cands = it["cands"]
        oneL = cands.get("L") and int(cands["L"]["rows"]) == 1
        oneR = cands.get("R") and int(cands["R"]["rows"]) == 1
        oneF = cands.get("F") and int(cands["F"]["rows"]) == 1

        if oneL or oneR:
            # 可参与左右同层
            it["h1"] = float((cands["L"] if oneL else cands["R"])["H"])
            if oneL and (not oneR):
                pairable_L.append(it)
            elif oneR and (not oneL):
                pairable_R.append(it)
            else:
                flexible.append(it)
        else:
            # 多行或只能全宽：独占一层（避免横切线误伤另一侧）
            best_any = None
            for k in ("L", "R", "F"):
                ...
                if best_any is None:
                    ...
                    # 先 H 小，再 rows 小
                    if (cand["H"], cand["rows"]) < (best_any[1]["H"], best_any[1]["rows"]):
                        best_any = (k, cand)
            if best_any is None:
                continue
            it["solo_mode"] = best_any[0]
            it["solo_cand"] = best_any[1]
            solo.append(it)

    # 高的先配，减少空浪费
    pairable_L.sort(key=lambda x: -float(x["h1"]))
    pairable_R.sort(key=lambda x: -float(x["h1"]))
    flexible.sort(key=lambda x: -float(x["h1"]))
    solo.sort(key=lambda x: -float(x["solo_cand"]["H"]))
    ...
    for it in solo:
        ...
        cand = it["solo_cand"]
        if mode == "L":
            strips.append({"left": {"seg": it["seg"], "cand": cand}, "right": None, "full": None})
        elif mode == "R":
            strips.append({"left": None, "right": {"seg": it["seg"], "cand": cand}, "full": None})
        else:
            strips.append({"left": None, "right": None, "full": {"seg": it["seg"], "cand": cand}})

    # 再配对单行块（同层左右）
    while pairable_L or pairable_R or flexible:
        left = None
        right = None

        if pairable_L:
            it = pairable_L.pop(0)
            left = {"seg": it["seg"], "cand": it["cands"]["L"]}
        elif flexible:
            it = flexible.pop(0)
            # flexible 优先塞左，留右给“只能右”的
            left = {"seg": it["seg"], "cand": it["cands"]["L"] if it["cands"].get("L") else it["cands"]["R"]}

        if pairable_R:
            it = pairable_R.pop(0)
            right = {"seg": it["seg"], "cand": it["cands"]["R"]}
        elif flexible:
            it = flexible.pop(0)
            right = {"seg": it["seg"], "cand": it["cands"]["R"] if it["cands"].get("R") else it["cands"]["L"]}

        strips.append({"left": left, "right": right, "full": None})

    # 计算层高
    for st in strips:
        st["H"] = _mix_strip_height(st)

    return strips

def _pack_strips_to_sheets(strips, max_h_mm=1500.0):
    """
    把 strips（层）FFD 装进多张纸（每张<=1500）
    """
    segs = list(strips)
    segs.sort(key=lambda s: -float(s.get("H", 0.0)))
    ...
    for st in segs:
        ...
        placed = False
        for sh in sheets:
            if sh["used_h"] + h <= float(max_h_mm) + 1e-9:
                sh["strips"].append(st)
                sh["used_h"] += h
                placed = True
                break
        if not placed:
            sheets.append({"strips": [st], "used_h": h})
    return sheets

def _render_one_mix_sheet_doc(sheet, is_contour):
    """
    输出一张混拼纸（单页），高度按 used_h 截断，避免底部大空白
    """
    W = float(SINGLE_W)
    H = float(min(SINGLE_H_MAX, max(30.0, sheet["used_h"])))
    ...
    yoff = 0.0
    for idx, st in enumerate(sheet["strips"]):
        # ✅ 层与层之间：画一条横切刀线（左刀一刀到右边缘）
        if idx > 0:
        ...
        if st.get("full"):
            seg = st["full"]["seg"]
            cand = st["full"]["cand"]
            img = seg["img_cont"] if is_contour else seg["img_body"]
            _draw_mix_block(page, seg, cand, img, yoff_mm=yoff)
        else:
            if st.get("left"):
                seg = st["left"]["seg"]
                cand = st["left"]["cand"]
        ...
        # ✅ 层底也画一条横切线（最后一层底边就是纸边；这里画不画都行）
        # 如果你不想多一条线，注释掉下一行：
        # _draw_horizontal_cutline(page, yoff + float(st["H"]), lw=1.0)
...
# safe_save
# =========================
# MIX: 从左往右（LR-SHELF），先大后小，底部裁切
# =========================
MIX_LEFT_X0 = float(SINGLE_MARGIN_L)                   # 5
MIX_LEFT_X1 = float(SINGLE_SPLIT_X)                    # 320
MIX_RIGHT_X0 = float(SINGLE_SPLIT_X + SINGLE_SPLIT_GAP_W)  # 330
MIX_RIGHT_X1 = float(SINGLE_W - SINGLE_MARGIN_R)       # 595

def _mix_item_sort_key(it):
    seg = it["seg"]
    area = float(seg.get("outer_w", 0.0)) * float(seg.get("outer_h", 0.0))
    n = float(seg.get("N", 1))
    return (-area * n, -area, -max(float(seg.get("outer_w", 0.0)), float(seg.get("outer_h", 0.0))))

def _mix_best_cand_for_width(seg, max_width):
    """
    给定当前行剩余宽度 max_width，给这个类型(seg)选一个最合适的块：
    - 允许旋转（ori=0/1）
    - cols = min(N, floor(max_width / w))
    - rows = ceil(N/cols)
    - block_w = cols*w
    - block_h = QR_BAND + rows*h
    评价：优先 rows=1，其次 block_w 大（更贴满），再其次 block_h 小
    """
    N = int(seg.get("N", 0))
    if N <= 0:
        return None

    ow = float(seg.get("outer_w", 0.0))
    oh = float(seg.get("outer_h", 0.0))
    if ow <= 0 or oh <= 0:
        return None

    best = None
    for ori in (0, 1):
        w = ow if ori == 0 else oh
        h = oh if ori == 0 else ow

        cols = int(math.floor(float(max_width) / float(w))) if w > 0 else 0
        cols = min(cols, N)
        if cols <= 0:
            continue

        rows = int(math.ceil(float(N) / float(cols)))
        block_w = float(cols) * float(w)
        block_h = float(QR_BAND) + float(rows) * float(h)
        ...
        for idx in range(N):
            ...
            placements.append({
                "x": float(cc) * float(w),
                "y": float(QR_BAND) + float(rr) * float(h),
                "w": float(w),
                "h": float(h),
                "rot": (ori == 1),
            })
        ...
        cand = {
            ...
            "placements": placements,  # 相对块左上角(0,0)
        }

        if best is None:
            best = cand
        else:
            key_new = (cand["rows"] != 1, -cand["block_w"], cand["block_h"])
            key_old = (best["rows"] != 1, -best["block_w"], best["block_h"])
            if key_new < key_old:
                best = cand

    return best

def _draw_mix_block_lr(page, seg, cand, img_bytes, xoff_mm, yoff_mm):
    """
    在 (xoff_mm, yoff_mm) 作为块左上角，画一个类型块：
    - 左上角 label
    - 右上角 QR
    - 图片在 QR_BAND 下方开始贴
    """
    bw = float(cand["block_w"])

    # QR：块右上角
    if seg.get("qr_bytes"):
        qr_rect = fitz.Rect(
            mm_to_pt(float(xoff_mm) + bw - QR_W), mm_to_pt(float(yoff_mm)),
            mm_to_pt(float(xoff_mm) + bw), mm_to_pt(float(yoff_mm) + QR_H)
        )
        page.insert_image(qr_rect, stream=seg["qr_bytes"], keep_proportion=True)
    ...
    label_text = seg.get("label_text") or ""
    if label_text:
        _draw_label_top_left(page, label_text, float(xoff_mm), float(yoff_mm), max_w_mm=max(10.0, bw - QR_W))

    # 贴图
    for p in cand["placements"]:
        x = float(xoff_mm) + float(p["x"])
        y = float(yoff_mm) + float(p["y"])
        w = float(p["w"])
        h = float(p["h"])
        ...
        if img_bytes:
            page.insert_image(inner, stream=img_bytes, keep_proportion=True, rotate=(90 if p.get("rot") else 0))

def _pack_mix_items_lr_shelf(mix_items, max_h_mm=1500.0):
    """
    ✅ 核心：从左往右排布
    - 每一行：先左区(5~320)，放不下就转右区(330~595)
    - 每个位置：从“剩余类型”里按大到小扫描，找到第一个能塞进剩余宽度的（大的放不下就换小的）
    - 行放不下任何：换区；区也放不下：换行；行也放不下高度：换页
    """
    remain = list(mix_items)
    remain.sort(key=_mix_item_sort_key)
    ...
    while remain:
        ...
        guard = 0
        while guard < 200000 and remain:
            guard += 1

            # 当前区剩余宽
            x1 = MIX_LEFT_X1 if region == "L" else MIX_RIGHT_X1
            rem_w = float(x1) - float(x)

            # 在当前“槽位”里：从大到小找一个能放下的
            pick = None
            for i, it in enumerate(remain):
                cand = _mix_best_cand_for_width(it["seg"], rem_w)
                if not cand:
                    continue
                # 放进去后本行高度会变成 max(row_h, cand.block_h)
                new_row_h = max(float(row_h), float(cand["block_h"]))
                if float(y) + new_row_h <= float(max_h_mm) + 1e-9:
                    pick = (i, it, cand)
                    break
            ...
            if pick:
                ...
                x = float(x) + float(cand["block_w"])
                row_h = max(float(row_h), float(cand["block_h"]))
                sh["used_h"] = max(float(sh["used_h"]), float(y) + float(row_h))

                # 删除已放的类型块
                del remain[i]
                continue

            # 当前区放不下任何一个块：先转右区，再换行，再换页
            if region == "L" and (not used_right):
                region = "R"
                x = MIX_RIGHT_X0
                used_right = True
                continue
            ...
            # 换行
            if row_h > 1e-9:
                y = float(y) + float(row_h)
                if y >= float(max_h_mm) - 1e-9:
                    break
                region = "L"
                x = MIX_LEFT_X0
                used_right = False
            ...
            # 行高=0且左右都试过了：说明这一页放不进任何剩余块（极端：块高度>1500）
            break

        # 本页没放任何东西 -> 防死循环：丢弃一个最大块
        if not sh["items"]:
            remain.pop(0)
            continue
...
def _render_one_mix_sheet_doc_lr(sheet, is_contour):
    # ✅ 底部裁掉：高度=used_h
    W = float(SINGLE_W)
    H = float(min(SINGLE_H_MAX, max(30.0, float(sheet.get("used_h", 0.0)))))
    ...
    items.sort(key=lambda t: (float(t["yoff"]), float(t["xoff"])))
    ...
    for it in items:
        ...
        yoff = float(it["yoff"])
        img = seg["img_cont"] if is_contour else seg["img_body"]
        _draw_mix_block_lr(page, seg, cand, img, xoff_mm=xoff, yoff_mm=yoff)
...
def run(cfg=None, input_pdfs=None, progress_cb=None, log_cb=None):
    ...
    for idx, path in enumerate(archived_paths, start=1):
        ...
        try:
            ...
            # ✅ N < 10：进入混拼（不再 skip）
            is_mix = (MIX_ENABLE and int(N) < MIX_THRESHOLD)
            ...
            for pi in range(pair_count):
                ...
                if is_mix:
                    # 计算混拼候选（含旋转/左区/右区/全宽）
                    cands = solve_mix_candidates(outer_w, outer_h, int(N))

                    seg = {
                        "type_key": tid,
                        "tid": tid,
                        "label_text": label_text,  # 标号（左上）
                        "qr_bytes": qr_bytes,  # QR（右上）
                        "img_body": img_body,
                        "img_cont": img_cont,
                        "N": int(N),
                    ...
                    _log(log_cb, "🧩 MIX_POOL: %s N=%d A=%.1f B=%.1f" % (tid, int(N), float(outer_w), float(outer_h)))
                    continue
    ...
    if global_single_segments:
        ...
        for sh in packed:
            ...
            page_h = float(min(SINGLE_H_MAX, max(30.0, sh["used_h"])))
            doc1 = _render_one_single_sheet_doc(sh, is_contour=False, fixed_h_mm=page_h)
            doc2 = _render_one_single_sheet_doc(sh, is_contour=True, fixed_h_mm=page_h)
    ...
    # =========================
    # 输出 mix_i：把 N<10 的类型做混拼
    # =========================
    mix_outputs_p1 = []
    mix_outputs_p2 = []

    if mix_items:
        sheets = _pack_mix_items_lr_shelf(mix_items, max_h_mm=SINGLE_H_MAX)

        mix_counter = 0
        for sh in sheets:
            mix_counter += 1
            name = "mix_%d.pdf" % mix_counter
            out_path1 = os.path.join(DEST_DIR1, name)
            out_path2 = os.path.join(DEST_DIR2, name)

            doc1 = _render_one_mix_sheet_doc_lr(sh, is_contour=False)
            doc2 = _render_one_mix_sheet_doc_lr(sh, is_contour=True)
            ...
            _log(log_cb, "🧩 %s：blocks=%d used_h=%.1f/%d  %s | %s"
                 % (name, len(sh.get("items") or []), sh.get("used_h", 0.0), int(SINGLE_H_MAX), p1, p2))
